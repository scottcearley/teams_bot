...
2. Give the Connector a name (`SUSDHelpDeskFAQP Connector`), an image and a short and long description. You also need to add your company/Teams Apps website.
3. As *Configuration page for your Connector*  add the following URL, which is used to connect and configure your Connector (`https://faqp.azurewebsites.net/susdHelpDeskFaqpConnector/config.html`).
4. In the *Valid domains* box enter the Teams Apps website (`https://faqp.azurewebsites.net`).
5. Choose *Yes* on the *enable actions on your Connector Cards* question.
6. As *Actions URL* add the Connector API endpoint (`https://faqp.azurewebsites.net/api/connector`).
...
* `./src/susdHelpDeskFaqpConnector/SusdHelpDeskFaqpConnector.ts` - implementation of the Connector that manages registering new subscriptions (`Connect()`) as well as a sample method to send a message to all subscribers (`Ping()`).
* `./src/web/susdHelpDeskFaqpConnector/config.html` - the configuration of the Connector .
* `./src/scripts/susdHelpDeskFaqpConnectorConfig.tsx` - React component of the configuration page page.
