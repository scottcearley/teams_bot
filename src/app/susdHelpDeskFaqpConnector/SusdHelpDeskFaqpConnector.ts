 ...
 */
interface ISusdHelpDeskFaqpConnectorData {
    webhookUrl: string;
    user: string;
    appType: string;
    groupName: string;
...
/**
 * Implementation of the "SusdHelpDeskFaqpConnectorConnector" Office 365 Connector
 */
@ConnectorDeclaration(
    "/api/connector/connect",
    "/api/connector/ping"
)
@PreventIframe("/susdHelpDeskFaqpConnector/config.html")
export class SusdHelpDeskFaqpConnector implements IConnector {
    ...
    public Ping(req: Request): Array<Promise <void>> {
        ...
        try {
            this.connectors.push("/connectors",
                (this.connectors.getData("/connectors") as ISusdHelpDeskFaqpConnectorData[]).filter(((c) => {
                    return c.existing;
                })));
        } catch (error) {
        ...
        return (this.connectors.getData("/connectors") as ISusdHelpDeskFaqpConnectorData[]).map((connector, index) => {
